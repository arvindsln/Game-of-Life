...
public class InfoPanel extends JPanel implements Constants{
	...
	public InfoPanel(Field field) {
		...
		JLabel totalCellsLabel = createLabel(TOTTAL_CELLS_LABEL, JLabel.RIGHT_ALIGNMENT);
		left.add(totalCellsLabel);
		JLabel aliveLabel = createLabel(ALIVE_CELLS_LABEL, JLabel.RIGHT_ALIGNMENT);
		left.add(aliveLabel);
		JLabel generationLabel = createLabel(GENERATION_LABEL, JLabel.RIGHT_ALIGNMENT);
		left.add(generationLabel);
		
		// labels values
		JPanel right = createPanel(CONTROL_PANEL_COLOUR, BoxLayout.Y_AXIS);
		JLabel totalCells = createLabel(String.valueOf(ROWS*COLS), JLabel.LEFT_ALIGNMENT);
		right.add(totalCells);
		JLabel aliveCells = createLabel(NUMER_LABEL, JLabel.LEFT_ALIGNMENT);
		right.add(aliveCells);
		JLabel generation = createLabel(NUMER_LABEL, JLabel.LEFT_ALIGNMENT);
